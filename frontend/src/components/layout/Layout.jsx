...
const navItems = [
  ...
  { path: '/vaccines', label: 'Vaccins', icon: 'syringe', perm: 'animals', module: 'vaccine_protocols' },
  { section: 'Gestion' },
  ...
  { path: '/accounting', label: 'Comptabilite', icon: 'book', module: 'accounting', roles: ['admin', 'accountant'] },
...
export default function Layout({ children }) {
  const { user, setUser, logout, hasModule } = useAuth();
  ...
  const filteredNavItems = navItems.filter(item => {
    ...
    if (item.module && !hasModule(item.module)) return false;   // paid module not unlocked
