...
Optional features are sold as **modules** unlocked per tenant by a **cryptographically signed license** (Ed25519), never a plain env flag. Single source of truth: `app/core/licensing.py` (`ALL_MODULES`, `MODULE_LABELS`). Current keys: `invoice_ninja`, `sms`, `google_calendar`, `accounting`, `vaccine_protocols`.
...
- **Issuing licenses** (deployer only, private key off the servers): `python -m app.licensing keygen`, then `python -m app.licensing sign --key private.pem --tenant <slug> --modules sms,invoice_ninja --max-users 5 --days 365` (a cap-only license with no modules is allowed).
- **Seat cap.** Max active users the admin can create (admin included): `MAX_USERS` env (dev / unsigned stack) or the license's `max_users` claim (production, tamper-proof), 0 = unlimited. Resolved into `TenantContext.max_users` (`resolve_max_users`) and enforced in `POST /auth/register` before any PocketBase side effect; surfaced to the SPA via `/auth/modules` + the session.
...
- **`accounting` module — cash closing + accounting export** (`app/api/endpoints/accounting.py`, admin/accountant): a daily Z-report (`CashRegisterClosing`) reconciles counted vs expected cash and **locks the day** (`record_payment` rejects payments once today is closed; `CashMovement` for non-invoice cash). Exports the period as a readable journal workbook + a FEC file (`app/core/accounting_export.py`). NB: invoices/payments are now dated with `date.today()` (app-local) rather than the DB's UTC `current_date`, so day aggregation is consistent.
- **`vaccine_protocols` module — "vaccins avancés"** (`app/api/endpoints/vaccination.py`): admins configure **protocols** (ordered `VaccineProtocolDose` series per species, each dose = valence + interval-from-previous + optional recurring booster); vets record a `Vaccination` against a protocol dose and the engine (`compute_next_due`) schedules `next_due_date`/`next_label` (next dose, or the booster recurrence). `GET /vaccinations/due` is the live schedule (latest administration per (animal, valence), within a window or overdue); `send_due_vaccination_reminders` (in `app/core/reminders.py`, scheduler-driven, dedup via `reminder_sent`) e-mails owners (+SMS if the `sms` module is on). Models auto-create at startup. Frontend: `VaccinesPage` (dus + protocoles) + a gated "Vaccins" tab on the animal sheet.
